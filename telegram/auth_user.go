...
func (c *Client) AuthPassword(ctx context.Context, password string) error {
	p, err := tg.AccountGetPassword(ctx, c)
	if err != nil {
	...
	auth, err := tg.AuthCheckPassword(ctx, c, &tg.InputCheckPasswordSRP{
		SRPID: p.SRPID,
		A:     a.A,
		M1:    a.M1,
	})
...
func (c *Client) AuthSendCode(ctx context.Context, phone string, options SendCodeOptions) (codeHash string, err error) {
	...
	sentCode, err := tg.AuthSendCode(ctx, c, &tg.AuthSendCodeRequest{
		PhoneNumber: phone,
		APIID:       c.appID,
		APIHash:     c.appHash,
		Settings:    settings,
...
func (c *Client) AuthSignIn(ctx context.Context, phone, code, codeHash string) error {
	a, err := tg.AuthSignIn(ctx, c, &tg.AuthSignInRequest{
		PhoneNumber:   phone,
...
func (c *Client) AuthAcceptTOS(ctx context.Context, id tg.DataJSON) error {
	_, err := tg.HelpAcceptTermsOfService(ctx, c, id)
	return err
}
...
func (c *Client) AuthSignUp(ctx context.Context, s SignUp) error {
	auth, err := tg.AuthSignUp(ctx, c, &tg.AuthSignUpRequest{
		LastName:      s.LastName,
